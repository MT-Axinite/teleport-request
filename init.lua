...
local function tpr_send(sender, receiver)
	if not mana.checkmana(sender, 1.5) then return end
...
local function tpj(player,param, drainfactor)
	if not mana.checkmana(player, drainfactor) then return end
...
local function tpe(player)
	if not mana.checkmana(player) then return end

	minetest.chat_send_player(player, "EVADE!")
	local mindistance = 15
	local maxdistance = 50
	local times = math.random(2, 6) -- how many times to jump - minimum,maximum
